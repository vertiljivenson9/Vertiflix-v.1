...
import { X, Play, Pause, Volume2, VolumeX, RotateCw, Subtitles, Languages, ChevronDown } from 'lucide-react'
...
const SUBTITLES: Record<SubtitleLang, string[]> = {
  es: ['Esta es una película increíble...', 'Los actores hacen un gran trabajo.', 'El final te sorprenderá.'],
  en: ['This is an amazing movie...', 'The actors do a great job.', 'The ending will surprise you.'],
  fr: ["C'est un film incroyable...", 'Les acteurs font un excellent travail.', 'La fin vous surprendra.']
}
...
export default function CoverPlayer({ movie, onClose }: CoverPlayerProps) {
  ...
  const getSubtitle = () => SUBTITLES[subtitleLang][Math.floor(progress / 30) % 3]
      ...
      return (
        <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#0088cc] to-[#0055aa]">
          <div className="text-center">
            ...
            <a href={url} target="_blank" rel="noopener" className="bg-white text-[#0088cc] px-6 py-2 rounded-full font-medium">Ver en Telegram</a>
          </div>
        </div>
      )
    }
        ...
        <div className="absolute inset-0 bg-black">
          ...
          {showSubtitles && isPlaying && (
            <div className="absolute bottom-20 left-0 right-0 text-center px-4">
              <span className="bg-black/80 text-white px-4 py-2 rounded text-lg">{getSubtitle()}</span>
            </div>
          )}
        </div>
      )}
      ...
      <div className={`absolute inset-0 transition-opacity duration-300 ${showControls || !isPlaying ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className="absolute top-0 left-0 right-0 p-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              {showLangMenu && (
                <div className="absolute top-full right-0 mt-1 bg-gray-900 rounded-lg overflow-hidden z-10">
                  {(['es', 'en', 'fr'] as SubtitleLang[]).map(l => (
                    <button key={l} onClick={() => { setSubtitleLang(l); setShowLangMenu(false) }} className={`block w-full px-4 py-2 text-sm text-left ${subtitleLang === l ? 'text-red-500' : 'text-white'} hover:bg-gray-800`}>
                      {LANG_NAMES[l]}
                    </button>
                  ))}
                </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setIsRevealed(!isRevealed)} className="flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded text-white text-sm">
